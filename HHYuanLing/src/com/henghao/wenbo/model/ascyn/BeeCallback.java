package com.henghao.wenbo.model.ascyn;
...
import com.benefit.buy.library.http.query.util.AQUtility;
...
public class BeeCallback<T> extends AjaxCallback<T> {
    ...
    public void async(Context context) {
        setAgent(ToolsKit.getDeviceInfo(context));
        super.async(context);
    }
    ...
    public String toString() {
        ...
        message = "消息：" + getUrl();
        if (null != params) {
            msgDesc += "请求：" + params.toString() + "\n\n";
            requset = "请求：" + params.toString();
        }
        else {
            msgDesc += "请求：{}\n\n";
            requset = "请求：" + "{}";
        ...
        if (null != result) {
            msgDesc += "响应：" + getResult().toString() + "\n\n";
            response = "响应：" + getResult().toString();
            float f = getResult().toString().getBytes().length;
            if (getResult().toString().getBytes().length > 1024) {
                ...
                msgDesc += "网络包大小：" + getResult().toString().getBytes().length + "b";
                netSize = "网络包大小：" + getResult().toString().getBytes().length + "b";
            }
        }
        return msgDesc;
    }
