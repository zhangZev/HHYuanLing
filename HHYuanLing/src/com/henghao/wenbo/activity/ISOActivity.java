...
import com.henghao.wenbo.adapter.NotPollingAdapter;
import com.henghao.wenbo.model.entity.TreeEntity;
import com.henghao.wenbo.utils.DeviceControl;
import com.henghao.wenbo.utils.LocationUtils;
import com.henghao.wenbo.views.dialog.DialogAlertEdit;
			...
			public void handleMessage(Message msg) {
				...
				if (msg.what == 1) {
					...
					for (int k = 0; k < tmp.length; k++) {
						...
						Intent intent = new Intent();
						for (int i = 0; i < ISOActivity.this.mList.size(); i++) {
							if (tmp[k].toString().contains(ISOActivity.this.mList.get(i).getNfcNo())) {
								if (ISOActivity.this.mList.get(i).isCheck()) {
									ToolsToast.show(ISOActivity.this, "该树已巡检");
									return;
								}
								ISOActivity.this.mList.get(i).setCheck(true);
...
//						GetUpdateNfc(tmp[k]);
				...
				/*
				 * adapter = new ArrayAdapter<EpcDataBase>(IsoActivity.this, android.R.layout.simple_list_item_1, firm);
	...
	private void initComentData() {
		...
		mEntity.setNfcNo("e2 00 51 09 67 13 01 17 08 20 c8 1c");
		mEntity.setNfcId("8b-5a-9f-7a-00-01-04-e0");
...
//			Log.e("as3992", "search retry");
	...
	void resolveIntent(Intent intent) {
		...
		if (NfcAdapter.ACTION_TECH_DISCOVERED.equals(action)) {
			...
			if (idString.equals("b1-cf-8d-2b-50-01-04-e0")) {
				intent2.putExtra(Constant.INTNET_TYPE, 1);
				intent2.setClass(ISOActivity.this, XiaoFangDetailActivity.class);
				startActivity(intent2);
				return;
			}
			else if (idString.equals("a7-b3-8d-2b-50-01-04-e0")) {
				intent2.putExtra(Constant.INTNET_TYPE, 2);
				intent2.setClass(ISOActivity.this, XiaoFangDetailActivity.class);
				startActivity(intent2);
				return;
			}
			else if (idString.equals("ac-d0-8d-2b-50-01-04-e0")) {
				intent2.putExtra(Constant.INTNET_TYPE, 2);
				intent2.setClass(ISOActivity.this, CommonWebActivity.class);
				intent2.putExtra(Constant.INTNET_URL, "http://www.baidu.com");
				intent2.putExtra(Constant.INTNET_TITLE, "百度页面");
				startActivity(intent2);
				return;
			}
			if (this.idStrings.contains(idString)) {
				ToolsToast.show(ISOActivity.this, "该树已巡检");
				return;
			}
			if (!this.idStrings.contains(idString)) {
				for (int i = 0; i < ISOActivity.this.mList.size(); i++) {
					if (idString.contains(ISOActivity.this.mList.get(i).getNfcId())) {
						...
						UpdateVIew(ISOActivity.this.mList.get(i));
						// 第一个数:马尾松
						intent2.putExtra(Constant.INTNET_TYPE, 111);
						intent2.setClass(ISOActivity.this, ScanDetailActivity.class);
						intent2.putExtra(Constant.INTNET_DATA, ISOActivity.this.mList.get(i));
						startActivity(intent2);
					}
				}
			}
			this.idStrings.add(idString);
		}
	}
}
